(function() {
   ...
   ITImages.$inject = ["$resource","spa-demo.config.APP_CONFIG"];
   function ITImages($resource, APP_CONFIG) {
      var imagesResource = $resource(APP_CONFIG.server_url  + "/api/images/:id",{id:'@id'},{query: { cache:false, isArray:true }});
     // var imagesResource = $resource(APP_CONFIG.server_url  + "/api/images/:id");
     var thingsResource = $resource(APP_CONFIG.server_url  + "/api/get_things");
     var service = this;
